...
class NeighbourhoodGraphConvolution(Module):
    ...
    def __init__(self,
        ...
        self.conv_weights = nn.ModuleList([nn.Linear(
            in_feat_dim,
            out_feat_dim//n_kernels,
            #out_feat_dim,
            bias=bias) for i in range(n_kernels)])
    ...
    def forward(self, neighbourhood_features, neighbourhood_pseudo_coord):
        ...
        neighbourhood_features = neighbourhood_features.contiguous().view(
    ...
    def convolution(self, neighbourhood, weights):
        ...
        # weighted_neighbourhood = torch.stack(weighted_neighbourhood, dim=2)
        # convolved_features = torch.sum(weighted_neighbourhood, dim=2)
